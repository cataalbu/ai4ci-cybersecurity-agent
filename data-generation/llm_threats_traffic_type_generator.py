...
class Config:
    ...
    out_nginx: Optional[str] = "./logs/train/nginx_access.log"
    out_api: Optional[str] = "./logs/train/api_app.log"
    out_ufw: Optional[str] = "./logs/train/fw_ufw.log"

    # If True, write labels/metadata sidecar for evaluation
    evaluation: bool = True
    out_manifest: Optional[str] = "./logs/train/manifest.jsonl"

    # Optional run control (so you can generate finite datasets)
    max_windows: Optional[int] = None  # None = run forever


def make_cfg(split: str) -> Config:
    c = Config()
    ...
    # Split outputs
    c.out_nginx = f"./logs/{split}/nginx_access.log"
    c.out_api = f"./logs/{split}/api_app.log"
    c.out_ufw = f"./logs/{split}/fw_ufw.log"
    c.out_manifest = f"./logs/{split}/manifest.jsonl"

    if split == "train":
        c.seed = 7
        c.lines_per_batch = 10
        c.scenario_weights = {
            "healthy": 0.35,
            "port_scan": 0.18,
            "bruteforce": 0.17,
        ...
        c.seed = 7
        c.temperature = 0.25  # a bit more variety
        c.lines_per_batch = 10
        c.scenario_weights = {
            "healthy": 0.65,
            "port_scan": 0.10,
...
def normalize_ufw_line(cfg: Config, line: str) -> str:
...
def build_graph(cfg: Config, llm: ChatOpenAI, rng: random.Random):
    ...
    def generate(state: dict) -> dict:
        ...
        for name, prompt in state.get("prompts", {}).items():
            try:
                ...
                if name == "ufw":
                    lines = [normalize_ufw_line(cfg, ln) for ln in lines]
                outputs[name] = lines
    ...
    def write_logs(state: dict) -> dict:
        ...
        # Manifest (labels + helpful metadata)
        append_manifest(
            cfg,
            {
                ...
                "channels": sorted(list(outputs.keys())),            # e.g. ["api","nginx","ufw"]
                "lines": {k: len(v) for k, v in outputs.items()},    # line counts per channel
            },
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model", type=str, default="openai/gpt-oss-20b", help="Model name")
    p.add_argument("--split", choices=["train", "val", "test"], default=os.getenv("SPLIT", "train"))
    p.add_argument("--max-windows", type=int, default=None, help="Generate N windows then exit (default: infinite)")
    p.add_argument("--sleep", type=float, default=None, help="Sleep seconds between windows (overrides config)")
    p.add_argument("--seed", type=int, default=None, help="Override seed")
    p.add_argument("--start-iso", type=str, default=None, help="Override start time (UTC ISO), e.g. 2026-01-01T00:00:00Z")
    return p.parse_args()
...
def main() -> int:
    ...
    # Allow CLI override for model selection
    cfg.model = args.model

    if args.max_windows is not None:
        cfg.max_windows = args.max_windows
    if args.sleep is not None:
        cfg.sleep_s = float(args.sleep)
    ...
    app = build_graph(cfg, llm, rng)

    sim_t = parse_iso_utc(args.start_iso) if args.start_iso else datetime.now(timezone.utc)

    print(f"# Starting split={args.split} (model={cfg.model}, base_url={cfg.base_url})")
    ...
    if cfg.max_windows is not None:
        print(f"# Will stop after {cfg.max_windows} windows.")
    ...
    try:
        while True:
            ...
            i += 1
            if cfg.max_windows is not None and i >= cfg.max_windows:
                print(f"\n# Done. Generated {i} windows.")
                return 0
